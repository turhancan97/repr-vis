...
for MODEL in MODELS:
    ...
    for level in LEVELS:
        ...
        for obj in object_classes:
            # Filter for this object, this level, in Orbit orientation
            mask = [
                lvl == level
                and oc == obj
                and ori == 'Orbit'
            ...
            angles = torch.stack([a for a, m in zip(result['number'], mask) if m]).to(torch.float32) / 540.0
...
with open('results_circle_bg_complex.csv', 'w') as f:
...
ax2.set_title('Average Val Loss by Background Complexity Level', fontsize=14)
...
plt.tight_layout()
plt.savefig('circle_background_complexity_results.png', dpi=300, bbox_inches='tight')
plt.show()
